...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `C:\Users\jesus\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`C:\Users\jesus\.claude\projects\<project>\memory\`).
